...
const tipoDotColors: Record<string, string> = {
  processo_criado: 'bg-primary/80',
  audiencia_realizada: 'bg-accent',
  documento_enviado: 'bg-secondary/80',
  prazo_cumprido: 'bg-badge-ativo',
  anotacao: 'bg-badge-pendente',
  contato: 'bg-muted-foreground',
  peticao_protocolada: 'bg-primary',
  decisao_recebida: 'bg-destructive',
};

const poloColors: Record<string, string> = {
  reclamante: 'bg-primary/10 text-primary',
  reclamada: 'bg-destructive/10 text-destructive',
  reclamada_pf: 'bg-destructive/10 text-destructive',
  autor: 'bg-accent/10 text-accent',
  reu: 'bg-destructive/10 text-destructive',
  vitima: 'bg-badge-ativo/20 text-badge-ativo-foreground',
  investigado: 'bg-badge-pendente/20 text-badge-pendente-foreground',
  terceiro_interessado: 'bg-muted text-secondary-foreground',
};
...
const situacaoDot: Record<string, string> = {
  solto: 'bg-badge-ativo',
  preso_preventivo: 'bg-destructive',
  preso_definitivo: 'bg-destructive',
  liberdade_provisoria: 'bg-badge-pendente',
  monitorado_eletronico: 'bg-badge-pendente',
};
...
export default function ClienteDetailPage({ clientId, onBack, onNavigateProcessoDetail }: ClienteDetailPageProps) {
  ...
  if (!cliente) {
    return (
      <div className="flex flex-col items-center justify-center py-32">
        ...
        <button onClick={onBack} className="mt-4 text-sm text-primary hover:text-primary">← Voltar para Clientes</button>
      </div>
    );
  }
  ...
  const renderPrazo = (prazo: string) => {
    if (!prazo) return <span className="text-muted-foreground/80">—</span>;
    const diff = daysDiff(prazo);
    if (diff <= 0) return <span className="bg-destructive/20 text-destructive rounded-md px-2 py-0.5 text-xs font-semibold">VENCIDO</span>;
    if (diff <= 3) return <span className="bg-destructive/20 text-destructive rounded-md px-2 py-0.5 text-xs font-semibold inline-flex items-center gap-1"><Flame className="w-3 h-3" />{diff} {diff === 1 ? 'dia' : 'dias'}</span>;
    if (diff <= 7) return <span className="bg-badge-pendente/20 text-badge-pendente-foreground rounded-md px-2 py-0.5 text-xs font-medium">{diff} dias</span>;
    return <span className="text-sm text-muted-foreground">{formatDateBR(prazo)}</span>;
  };

  const renderAudiencia = (aud: string) => {
    if (!aud) return <span className="text-muted-foreground/80">—</span>;
    ...
    return (
      <div className="flex items-center gap-1">
        ...
        <div>
          <div className="text-sm text-foreground/80">{formatDateBR(aud)}</div>
      ...
      <div className="border-l-2 border-border ml-3">
        {display.map((atv, i) => (
          <div key={atv.id} className={`relative pl-4 ${i < display.length - 1 ? 'pb-4' : ''}`}>
            <span className={`absolute -left-[5px] top-0.5 w-2.5 h-2.5 rounded-full border-2 border-white ${tipoDotColors[atv.tipo] || 'bg-secondary/80'}`} />
            <div className="ml-1">
  ...
  const renderProcessosTable = (procs: Processo[], showEmpty = true) => {
    ...
    return (
      <table className="w-full">
        ...
        <tbody>
          {procs.map((proc) => (
            <tr key={proc.id} className="hover:bg-muted transition-colors border-b border-border/50 last:border-0">
              ...
              {activeTab === 'processos' && (
                <>
                  ...
                  <td className="px-4 py-3">
                    <span className="bg-muted/80 text-secondary-foreground text-xs font-medium px-2 py-0.5 rounded-md">{proc.tribunal}</span>
                  </td>
                </>
              )}
              ...
              <td className="px-4 py-3">
                <div className="relative">
                  <button onClick={() => setOpenDropdown(openDropdown === proc.id ? null : proc.id)} className="text-muted-foreground hover:text-secondary-foreground p-1">
                  ...
                  {openDropdown === proc.id && (
                    <div className="absolute right-0 top-8 bg-card border border-border shadow-lg rounded-lg py-1 z-10 w-44">
                      <button className="flex items-center gap-2 px-3 py-2 text-sm text-foreground/80 hover:bg-muted w-full" onClick={() => { setOpenDropdown(null); onNavigateProcessoDetail?.(proc.id); }}>
                        <Eye className="w-4 h-4" /> Ver Detalhes
                      </button>
                      <button className="flex items-center gap-2 px-3 py-2 text-sm text-foreground/80 hover:bg-muted w-full" onClick={() => setOpenDropdown(null)}>
                        <Scale className="w-4 h-4" /> Nova Audiência
                      </button>
                      <div className="my-1 border-t border-border/50" />
                      <button className="flex items-center gap-2 px-3 py-2 text-sm text-foreground/80 hover:bg-muted w-full" onClick={() => handleEncloseProcesso(proc.id)}>
  ...
  const renderDadosCadastrais = () => {
    ...
    fields.push({
      label: 'STATUS',
      value: (
        <span className={`inline-flex items-center text-xs font-medium px-2 py-0.5 rounded-full ${cliente.status === 'ativo' ? 'bg-badge-ativo/20 text-badge-ativo-foreground' : 'bg-muted/80 text-muted-foreground'}`}>
    ...
    if (cliente.practice_area === 'criminal') {
      ...
      fields.push({
        ...
        value: (
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${situacaoDot[c.situacao_prisional] || 'bg-secondary/80'}`} />
      ...
      <div className="flex flex-col gap-3">
        {fields.map((f, i) => {
          if (f.label === '' && f.value === null) {
            return <div key={i} className="border-t border-border/50 my-1" />;
          }
          if (f.label === '__SECTION__') {
            return <div key={i} className="border-t border-border/50 my-1"><p className="text-xs text-muted-foreground uppercase tracking-wide mt-2 mb-1">{f.value}</p></div>;
          }
          return (
            <div key={i}>
  ...
  const selectCls = 'bg-card border border-border rounded-md px-3 py-2 text-sm text-secondary-foreground focus:outline-none focus:ring-2 focus:ring-ring';
    ...
    <div className="bg-muted min-h-full -m-6">
      ...
      <div className="bg-card border-b border-border px-6 py-5 flex items-start justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {cliente.is_vip && (
                <span className="bg-badge-pendente/20 text-badge-pendente-foreground text-xs font-semibold px-2 py-0.5 rounded-full flex items-center gap-1">
            ...
            <div className="flex items-center gap-4 mt-1.5 flex-wrap text-sm text-muted-foreground">
              ...
              {polo && (
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${poloColors[polo] || 'bg-muted text-secondary-foreground'}`}>
                  {getPoloLabel(polo)}
                </span>
              )}
            </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button onClick={() => setSlideOpen(true)} className="border border-border text-secondary-foreground hover:bg-muted rounded-md px-3 py-1.5 text-sm flex items-center gap-1.5 transition-colors">
            <Edit className="w-4 h-4" /> Editar
          </button>
          <button className="border border-border text-secondary-foreground hover:bg-muted rounded-md px-3 py-1.5 text-sm flex items-center gap-1.5 transition-colors">
            <Mail className="w-4 h-4" /> E-mail
          </button>
          <button className="bg-muted/80 text-foreground/80 hover:bg-secondary rounded-md px-3 py-1.5 text-sm flex items-center gap-1.5 transition-colors">
            <MessageCircle className="w-4 h-4 text-green-500" /> WhatsApp
          </button>
          <button className="bg-primary text-white hover:bg-primary/90 rounded-md px-4 py-1.5 text-sm font-medium flex items-center gap-1.5 transition-colors">
            <Plus className="w-4 h-4" /> Novo Processo
          </button>
          <div className="flex items-center gap-1.5 ml-2">
            <span className={`w-2 h-2 rounded-full ${cliente.status === 'ativo' ? 'bg-badge-ativo' : 'bg-accent'}`} />
      ...
      <div className="px-6 py-4 bg-card border-b border-border">
        <div className="grid grid-cols-4 gap-4">
          <div className="bg-muted rounded-lg p-4 flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-primary/20 text-primary flex-shrink-0"><Briefcase className="w-4 h-4" /></div>
            <div>
          ...
          <div className="bg-muted rounded-lg p-4 flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-accent/10 text-accent flex-shrink-0"><Scale className="w-4 h-4" /></div>
            <div>
          ...
          <div className="bg-muted rounded-lg p-4 flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-badge-pendente/20 text-badge-pendente-foreground flex-shrink-0"><AlertCircle className="w-4 h-4" /></div>
            <div>
          ...
          <div className="bg-muted rounded-lg p-4 flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-badge-ativo/20 text-badge-ativo-foreground flex-shrink-0"><Smile className="w-4 h-4" /></div>
            <div>
        ...
        {([
          ...
          >
            ...
            {'badge' in tab && tab.badge !== undefined && (
              <span className="bg-primary/20 text-primary text-xs font-semibold px-1.5 py-0.5 rounded-full ml-1.5">{tab.badge}</span>
            )}
            {'comingSoon' in tab && tab.comingSoon && (
              <span className="bg-muted/80 text-muted-foreground text-xs px-1.5 py-0.5 rounded-full ml-1.5">Em breve</span>
            )}
          </button>
        ))}
      </div>
      ...
      <div className="p-6">
            ...
            <div className="col-span-7">
              <div className="bg-card rounded-lg border border-border shadow-sm">
                <div className="px-5 py-4 border-b border-border flex items-center justify-between">
                  ...
                  <button onClick={() => setActiveTab('processos')} className="text-xs text-primary hover:text-primary cursor-pointer">Ver todos</button>
                </div>
            ...
            <div className="col-span-5 space-y-6">
              ...
              <div className="bg-card rounded-lg border border-border shadow-sm">
                <div className="px-5 py-4 border-b border-border flex items-center justify-between">
                  ...
                  <button onClick={() => setSlideOpen(true)} className="text-xs text-primary cursor-pointer">Editar</button>
                </div>
              ...
              {cliente.notes ? (
                ...
                <div className="bg-muted border border-border/50 rounded-lg px-5 py-4 flex items-start gap-3">
                  <StickyNote className="w-4 h-4 text-muted-foreground/80 mt-0.5 flex-shrink-0" />
          ...
          <div>
            <div className="flex items-center gap-3 mb-4 flex-wrap">
              ...
              <div className="ml-auto">
                <button onClick={() => setAnnotationModal(true)} className="border border-border text-secondary-foreground hover:bg-muted rounded-md px-3 py-2 text-sm flex items-center gap-1.5 transition-colors">
        ...
        {activeTab === 'documentos' && (
          <div className="flex flex-col items-center justify-center gap-4 py-20">
            ...
            <h2 className="text-xl font-semibold text-secondary-foreground">Módulo de Documentos</h2>
            <p className="text-sm text-muted-foreground text-center max-w-xs">Este módulo estará disponível em breve.</p>
            <span className="bg-primary/10 text-primary text-xs font-medium px-3 py-1 rounded-full">Em desenvolvimento</span>
          </div>
        )}
      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-card rounded-xl shadow-xl p-6 max-w-sm mx-4 text-center">
              ...
              <div className="flex gap-3">
                <button onClick={() => setDeleteConfirm(false)} className="flex-1 border border-border rounded-md py-2 text-sm text-secondary-foreground hover:bg-muted transition-colors">Cancelar</button>
                <button onClick={handleDelete} className="flex-1 bg-destructive text-destructive-foreground rounded-md py-2 text-sm font-medium hover:bg-destructive/90 transition-colors">Excluir</button>
              </div>
            </div>
          </div>
        </>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-card rounded-xl shadow-xl w-full max-w-md mx-4">
              <div className="flex items-center justify-between px-6 py-4 border-b border-border">
                ...
                <button onClick={() => setAnnotationModal(false)} className="text-muted-foreground hover:text-secondary-foreground"><X className="w-5 h-5" /></button>
              </div>
              ...
              <div className="px-6 py-4 border-t border-border flex justify-end gap-3">
                <button onClick={() => setAnnotationModal(false)} className="text-sm text-secondary-foreground hover:text-foreground transition-colors">Cancelar</button>
                <button onClick={handleSaveAnnotation} className="bg-primary text-white text-sm font-medium rounded-md px-6 py-2 hover:bg-primary/90 transition-colors">Salvar</button>
              </div>
            </div>
          </div>
        </>
