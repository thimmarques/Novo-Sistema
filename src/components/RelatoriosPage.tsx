...
const statusBarColors: Record<string, string> = {
  ativo: 'bg-badge-ativo',
  audiencia: 'bg-badge-audiencia',
  pendente: 'bg-badge-pendente',
  recurso: 'bg-badge-recurso',
  acordo: 'bg-accent',
  encerrado: 'bg-badge-encerrado',
};
...
export default function RelatoriosPage() {
    ...
    return (
      <div key={reportType} className="bg-card border border-border rounded-lg p-5 hover:border-border hover:shadow-sm transition-all duration-150 cursor-pointer flex flex-col gap-4">
        <div className="flex items-start justify-between">
          ...
          <button onClick={() => { showToast('Exportação iniciada', 'info'); }} className="bg-muted border border-border hover:bg-muted/80 rounded-md px-2.5 py-1.5 text-xs text-secondary-foreground flex items-center gap-1">
        ...
        <button onClick={() => setActiveReport(reportType)} className="border border-border rounded-md py-2 text-sm text-secondary-foreground hover:bg-muted w-full text-center mt-auto flex items-center justify-center gap-1">
  ...
  const periodButtons = (
    <div className="flex gap-2 mb-4">
      {[['mes', 'Este mês'], ['3meses', '3 meses'], ['6meses', '6 meses'], ['ano', '1 ano'], ['todos', 'Todos']].map(([k, l]) => (
        <button key={k} onClick={() => setPeriodFilter(k)} className={`text-xs rounded-md px-2.5 py-1.5 cursor-pointer ${periodFilter === k ? 'bg-primary text-white' : 'bg-muted/80 text-secondary-foreground'}`}>{l}</button>
      ))}
    </div>
  );
  ...
  const renderProcessosPorStatus = () => {
    ...
    return (
      <div>
        ...
        {total > 0 && (
          <div className="h-3 rounded-full bg-muted/80 overflow-hidden flex mb-5">
            {statuses.filter(s => counts[s]).map(s => (
            ...
            <tbody>
              {statuses.filter(s => counts[s]).map(s => (
                <tr key={s} className="border-b border-border/50">
                  ...
                  <td className="px-4 py-3 text-sm text-secondary-foreground">{total > 0 ? ((counts[s] / total) * 100).toFixed(1) : 0}%</td>
        ...
        {areas.map(a => {
          ...
          return (
            <div key={a} className="bg-card rounded-lg p-4 text-center border border-border">
              ...
              <div className="h-1.5 rounded-full bg-muted/80 mt-3">
                <div className={`h-1.5 rounded-full ${a === 'trabalhista' ? 'bg-badge-trabalhista' : a === 'civil' ? 'bg-badge-civil' : a === 'criminal' ? 'bg-badge-criminal' : 'bg-badge-previdenciario'}`} style={{ width: `${Math.min(pct, 100)}%` }} />
              </div>
          ...
          <tbody>
            {financeiroMensal.map(m => (
              <tr key={m.month} className="border-b border-border/50">
                ...
                <td className="px-4 py-3 text-sm text-foreground/80">{formatBRL(m.honorarios)}</td>
                <td className="px-4 py-3 text-sm text-foreground/80">{formatBRL(m.despesas)}</td>
                <td className="px-4 py-3 text-sm text-foreground/80">{formatBRL(m.custas)}</td>
                <td className="px-4 py-3 text-sm font-semibold text-foreground">{formatBRL(m.total)}</td>
                <td className="px-4 py-3">
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${m.aboveAvg ? 'bg-badge-ativo/20 text-badge-ativo-foreground' : 'bg-badge-pendente/20 text-badge-pendente-foreground'}`}>
  ...
  const renderAudienciasPeriodo = () => {
    const aggs = [
      { label: 'Agendadas', count: audienciasAgendadas, color: 'text-primary' },
      ...
      { label: 'Canceladas', count: audienciasCanceladas, color: 'text-secondary-foreground' },
    ];
        ...
        <div className="bg-muted rounded-lg p-4 mb-4">
          {Object.entries(tipoCounts).map(([t, c]) => (
            <div key={t} className="flex items-center gap-3 mb-2 last:mb-0">
              <span className="text-xs text-secondary-foreground w-24">{tipoLabels[t] || t}</span>
              <div className="flex-1 bg-secondary rounded-full h-2">
                <div className="bg-primary/80 rounded-full h-2" style={{ width: `${totalAud > 0 ? Math.min((c / totalAud) * 100, 100) : 0}%` }} />
              </div>
  ...
  const renderInadimplencia = () => (
    <div>
      <div className="grid grid-cols-3 gap-3 mb-5">
        ...
        <div className="bg-destructive/10 rounded-lg p-3 text-center">
          <div className="text-xs text-destructive uppercase">Taxa Inadimplência</div>
          <div className="text-xl font-bold text-destructive">{taxaInadimplencia.toFixed(1)}%</div>
        </div>
        <div className="bg-muted rounded-lg p-3 text-center">
        ...
        {inadimplenciaPorAdvogado.filter(a => a.vencido > 0 || a.aReceber > 0).map(a => {
          ...
          return (
            <div key={a.id} className="bg-card border border-border rounded-lg p-4 flex items-center gap-4">
              ...
              <div className="w-32">
                <div className="h-1.5 rounded-full bg-muted/80 overflow-hidden flex">
                  <div className="bg-badge-ativo h-full" style={{ width: `${Math.min(taxa, 100)}%` }} />
                  {a.vencido > 0 && <div className="bg-destructive h-full" style={{ width: `${Math.min((a.vencido / Math.max(a.total, 1)) * 100, 100)}%` }} />}
                </div>
              </div>
            </div>
          );
            ...
            <tbody>
              {vencidosPorCliente.map(v => (
                <tr key={v.id} className="border-b border-border/50">
                  ...
                  <td className="px-4 py-3 text-sm text-secondary-foreground">{v.advogado?.name || '—'}</td>
                  <td className="px-4 py-3 text-sm font-semibold text-foreground">{formatBRL(v.valor)}</td>
                  <td className="px-4 py-3 text-sm text-secondary-foreground">{formatDateBR(v.vencimento)}</td>
                  <td className="px-4 py-3">
                    <span className={`text-sm font-medium ${v.diasAtraso > 90 ? 'text-destructive font-bold' : v.diasAtraso > 30 ? 'text-destructive/80' : 'text-accent'}`}>
                      {v.diasAtraso} dias
                    </span>
                  </td>
                </tr>
  ...
  const renderMeusPrazos = () => (
    <div>
      ...
      {prazosFatais.length > 0 ? (
        ...
        <div className="bg-badge-ativo/10 border border-badge-ativo/20 rounded-lg p-4 mb-4 flex items-center gap-2">
            ...
            {allProcessos.filter(p => p.prazo_fatal).sort((a, b) => daysDiff(a.prazo_fatal) - daysDiff(b.prazo_fatal)).map(p => {
              ...
              return (
                <tr key={p.id} className="border-b border-border/50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-sm font-medium ${diff <= 0 ? 'text-red-600 font-bold' : diff <= 3 ? 'text-red-600' : diff <= 7 ? 'text-amber-600' : 'text-foreground/80'}`}>
  ...
  const renderMinhasAudiencias = () => {
    ...
    return (
      <div>
        {periodButtons}
        <div className="grid grid-cols-3 gap-3 mb-5">
          <div className="bg-primary/10 rounded-lg p-3 text-center"><div className="text-2xl font-bold text-primary">{mesCount}</div><div className="text-xs text-muted-foreground">Este mês</div></div>
          <div className="bg-badge-ativo/10 rounded-lg p-3 text-center"><div className="text-2xl font-bold text-badge-ativo-foreground">{audienciasRealizadas}</div><div className="text-xs text-muted-foreground">Realizadas</div></div>
          <div className="bg-badge-pendente/10 rounded-lg p-3 text-center"><div className="text-2xl font-bold text-badge-pendente-foreground">{audienciasAgendadas}</div><div className="text-xs text-muted-foreground">Pendentes</div></div>
        </div>
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          <table className="w-full">
            <thead>
            ...
            <tbody>
              {audienciaEventos.sort((a, b) => a.data.localeCompare(b.data)).map(e => (
                <tr key={e.id} className="border-b border-border/50">
                  ...
                  <td className="px-4 py-3 text-sm text-secondary-foreground">{e.cliente_nome || '—'}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${e.audiencia_status === 'realizada' ? 'bg-badge-ativo/20 text-badge-ativo-foreground' : e.audiencia_status === 'cancelada' ? 'bg-muted/80 text-muted-foreground' : 'bg-primary/20 text-primary'}`}>
  ...
  const renderMeusClientes = () => (
    <div>
      <div className="grid grid-cols-4 gap-3 mb-5">
        <div className="bg-badge-ativo/10 rounded-lg p-3 text-center"><div className="text-2xl font-bold text-badge-ativo-foreground">{clientesPorStatus.ativo}</div><div className="text-xs text-muted-foreground">Ativos</div></div>
        <div className="bg-muted rounded-lg p-3 text-center"><div className="text-2xl font-bold text-secondary-foreground">{clientesPorStatus.inativo}</div><div className="text-xs text-muted-foreground">Inativos</div></div>
        <div className="bg-primary/10 rounded-lg p-3 text-center"><div className="text-2xl font-bold text-primary">{clientesPorTipo.PF}</div><div className="text-xs text-muted-foreground">Pessoa Física</div></div>
        <div className="bg-accent/10 rounded-lg p-3 text-center"><div className="text-2xl font-bold text-accent">{clientesPorTipo.PJ}</div><div className="text-xs text-muted-foreground">Pessoa Jurídica</div></div>
      </div>
    </div>
  );
  ...
  const reportIcons: Record<ReportType, { icon: React.ElementType; bg: string }> = {
    'processos-status': { icon: BarChart2, bg: 'bg-primary/10 text-primary' },
    'processos-area': { icon: PieChart, bg: 'bg-accent/10 text-accent' },
    'financeiro': { icon: TrendingUp, bg: 'bg-badge-ativo/10 text-badge-ativo-foreground' },
    'produtividade': { icon: Award, bg: 'bg-badge-pendente/10 text-badge-pendente-foreground' },
    'audiencias': { icon: Calendar, bg: 'bg-primary/10 text-primary' },
    'inadimplencia': { icon: AlertTriangle, bg: 'bg-destructive/10 text-destructive' },
    'meus-processos-status': { icon: BarChart2, bg: 'bg-primary/10 text-primary' },
    'meus-processos-area': { icon: Briefcase, bg: 'bg-accent/10 text-accent' },
    'minhas-audiencias': { icon: Scale, bg: 'bg-primary/10 text-primary' },
    'meus-prazos': { icon: Clock, bg: 'bg-badge-pendente/10 text-badge-pendente-foreground' },
    'meus-clientes': { icon: Users, bg: 'bg-badge-ativo/10 text-badge-ativo-foreground' },
    'atividade-recente': { icon: Activity, bg: 'bg-secondary/20 text-secondary-foreground' },
  };
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {admin && (
          <button onClick={() => showToast('Exportação iniciada', 'info')} className="flex items-center gap-1.5 border border-border text-secondary-foreground hover:bg-muted rounded-md px-3 py-2 text-sm">
      ...
      {/* Role banner */}
      {!admin && (
        <div className="bg-primary/10 border border-primary/20 rounded-lg px-4 py-3 mb-6 flex items-center gap-3">
          <Info className="w-4 h-4 text-primary/80 flex-shrink-0" />
          <div>
            <div className="text-sm text-primary">Você está visualizando apenas seus próprios dados.</div>
            <div className="text-xs text-primary/80 mt-0.5">Administradores têm acesso aos relatórios completos do escritório.</div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {admin ? (
          <>
            {renderCard(BarChart2, 'bg-primary/10 text-primary', 'Processos por Status', 'Visão geral de todos os processos agrupados por status atual',
            ...
            {renderCard(PieChart, 'bg-accent/10 text-accent', 'Distribuição por Área do Direito', 'Porcentagem de processos em cada área de atuação do escritório',
              <>{Object.entries(processosPorArea).map(([a, c]) => <span key={a}>{areaLabels[a] || a}: {c}</span>)}</>,
              'processos-area'
            )}
            {renderCard(TrendingUp, 'bg-badge-ativo/10 text-badge-ativo-foreground', 'Financeiro Mensal', 'Receitas, despesas e inadimplência dos últimos 6 meses',
            ...
            {renderCard(Award, 'bg-badge-pendente/10 text-badge-pendente-foreground', 'Produtividade por Advogado', 'Processos ativos, audiências e prazos por advogado do escritório',
              topPerformer ? <span>{topPerformer.name} — {topPerformer.count} processos ativos</span> : <span>—</span>,
              'produtividade'
            )}
            {renderCard(Calendar, 'bg-primary/10 text-primary', 'Audiências do Período', 'Audiências realizadas, agendadas e canceladas por período',
            ...
            {renderCard(AlertTriangle, 'bg-destructive/10 text-destructive', 'Análise de Inadimplência', 'Clientes com valores em atraso e taxa de inadimplência por advogado',
              <><span>Total vencido: {formatBRL(totalVencido)}</span><span>Taxa: {taxaInadimplencia.toFixed(1)}%</span></>,
              'inadimplencia',
              taxaInadimplencia > 10 ? <span className="bg-destructive/20 text-destructive text-xs rounded-full px-2 py-0.5">⚠ Atenção</span> : undefined
            )}
          </>
        ) : (
          <>
            {renderCard(BarChart2, 'bg-primary/10 text-primary', 'Meus Processos por Status', 'Seus processos agrupados por status atual',
              <><span>Ativos: {processosPorStatus['ativo'] || 0}</span><span>Pendentes: {processosPorStatus['pendente'] || 0}</span><span>Encerrados: {processosPorStatus['encerrado'] || 0}</span></>,
              'meus-processos-status'
            )}
            {renderCard(Briefcase, 'bg-accent/10 text-accent', 'Meus Processos por Área', 'Distribuição dos seus processos por área do direito',
              <>{currentUser!.practice_areas.map(a => <span key={a}>{areaLabels[a]}: {processosPorArea[a] || 0}</span>)}</>,
              'meus-processos-area'
            )}
            {renderCard(Scale, 'bg-primary/10 text-primary', 'Minhas Audiências', 'Histórico e agenda de audiências dos seus processos',
            ...
            {renderCard(Clock, 'bg-badge-pendente/10 text-badge-pendente-foreground', 'Meus Prazos', 'Prazos processuais e fatais dos seus processos',
              <>
                <span>Esta semana: {prazosEstaSemana}</span><span>Este mês: {prazosEsteMes}</span>
                {prazosFatais.length > 0 && <span className="bg-destructive/20 text-destructive text-xs rounded px-1.5">{prazosFatais.length} FATAIS</span>}
              </>,
              'meus-prazos'
            )}
            {renderCard(Users, 'bg-badge-ativo/10 text-badge-ativo-foreground', 'Meus Clientes', 'Visão dos seus clientes ativos e respectivos processos',
            ...
            {renderCard(Activity, 'bg-secondary/20 text-secondary-foreground', 'Atividade Recente', 'Histórico de ações realizadas nos seus processos',
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={() => { setActiveReport(null); setPeriodFilter('todos'); }}>
          ...
          <div className="bg-card rounded-xl shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col mx-4 relative z-10" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => showToast('Exportando relatório...', 'info')} className="border border-border text-secondary-foreground hover:bg-muted rounded-md px-3 py-1.5 text-sm flex items-center gap-1">
                  <Download className="w-4 h-4" /> Exportar CSV
                </button>
                <button onClick={() => { setActiveReport(null); setPeriodFilter('todos'); }} className="text-muted-foreground hover:text-secondary-foreground p-1"><X className="w-5 h-5" /></button>
              </div>
            </div>
