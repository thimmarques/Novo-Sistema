  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setShowNewModal(true)} className="flex items-center bg-primary text-white text-sm font-medium rounded-md px-4 py-2 hover:bg-primary/90 transition-colors">
        ...
        {tabs.map((t) => (
          <button
            ...
            className={`rounded-md px-4 py-1.5 text-sm cursor-pointer transition-colors ${
              activeTab === t.key ? 'bg-primary text-white font-medium' : 'text-muted-foreground hover:text-foreground/80 hover:bg-muted/80'
            }`}
          >
            {t.label}
          </button>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <select value={filterArea} onChange={(e) => setFilterArea(e.target.value)} className="border border-border rounded-md px-3 py-2 text-sm text-secondary-foreground focus:outline-none focus:ring-2 focus:ring-ring">
        ...
        <select value={filterAudTipo} onChange={(e) => setFilterAudTipo(e.target.value)} className="border border-border rounded-md px-3 py-2 text-sm text-secondary-foreground focus:outline-none focus:ring-2 focus:ring-ring">
      ...
      <div className="grid grid-cols-4 gap-3 mb-5">
        <div className="bg-card border border-border rounded-lg px-4 py-3 flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-muted flex items-center justify-center"><LayoutList className="w-4 h-4 text-secondary-foreground" /></div>
        ...
        <div className="bg-card border border-border rounded-lg px-4 py-3 flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-accent/10 flex items-center justify-center"><CalendarDays className="w-4 h-4 text-accent" /></div>
        ...
        {admin ? (
          <div className="bg-card border border-border rounded-lg px-4 py-3 flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg bg-destructive/10 flex items-center justify-center"><Flame className="w-4 h-4 text-destructive" /></div>
          ...
          <div className="bg-card border border-border rounded-lg px-4 py-3 flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg bg-badge-ativo/20 flex items-center justify-center"><CheckCircle className="w-4 h-4 text-badge-ativo-foreground" /></div>
                ...
                return (
                  <div key={aud.id} className="bg-card border border-border rounded-lg p-4 hover:border-border hover:shadow-sm transition-all duration-150 mb-3">
                    <div className="flex items-start gap-4">
                      {/* DATE BLOCK */}
                      <div className={`flex-shrink-0 rounded-lg p-3 text-center w-16 ${isT ? 'bg-primary' : isUrgent ? 'bg-badge-pendente/20' : 'bg-muted'}`}>
                        <div className={`text-3xl font-bold leading-none ${isT ? 'text-white' : isUrgent ? 'text-amber-600' : 'text-foreground'}`}>
                          {d.getDate()}
                        </div>
                        <div className={`text-xs uppercase mt-1 ${isT ? 'text-primary-foreground' : 'text-muted-foreground'}`}>
                          {MONTH_NAMES[d.getMonth()].slice(0, 3)}
                        </div>
                        <div className={`text-xs ${isT ? 'text-primary-foreground' : 'text-muted-foreground'}`}>{d.getFullYear()}</div>
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {aud.numero_cnj && <span className="text-muted-foreground/80">·</span>}
                          {aud.practice_area && (
                            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${areaColors[aud.practice_area] || 'bg-muted/80 text-secondary-foreground'}`}>
                        ...
                        {aud.cliente_nome && (
                          <div className="flex items-center gap-1.5 mt-1">
                            ...
                            <span className="text-sm text-secondary-foreground">{aud.cliente_nome}</span>
                          </div>
                        )}
                        {aud.local && (
                          <div className="flex items-center gap-1.5 mt-1">
                        ...
                        <div className="border-t border-border/50 mt-3 pt-3 flex items-center gap-2 flex-wrap">
                          {lawyer && (
                            <>
                          ...
                          {aud.audiencia_tipo === 'virtual' && <Video className="w-3.5 h-3.5 text-primary ml-1" />}
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col items-end gap-2 flex-shrink-0 ml-2">
                        ...
                        <div className="relative">
                          <button onClick={() => setOpenDropdown(openDropdown === aud.id ? null : aud.id)} className="text-muted-foreground hover:text-secondary-foreground p-1">
                          ...
                          {openDropdown === aud.id && (
                            <div className="absolute right-0 top-8 bg-card border border-border rounded-lg shadow-lg py-1 z-20 w-48">
                              {aud.processo_id && onNavigateProcessoDetail && (
                                <button onClick={() => { setOpenDropdown(null); onNavigateProcessoDetail(aud.processo_id); }} className="w-full text-left px-3 py-2 text-sm text-foreground/80 hover:bg-muted flex items-center gap-2">
                                  <Briefcase className="w-4 h-4" /> Ver Processo
                                </button>
                              )}
                              {aud.audiencia_status === 'agendada' && (
                                <button onClick={() => handleRegistrarResultado(aud)} className="w-full text-left px-3 py-2 text-sm text-badge-ativo-foreground hover:bg-muted flex items-center gap-2">
                                  <CheckCircle className="w-4 h-4" /> Registrar Resultado
                                </button>
                              )}
                              <div className="border-t border-border/50 my-1" />
                              <button onClick={() => setOpenDropdown(null)} className="w-full text-left px-3 py-2 text-sm text-foreground/80 hover:bg-muted flex items-center gap-2">
                                <Scale className="w-4 h-4" /> Editar
                              </button>
                              <button onClick={() => setOpenDropdown(null)} className="w-full text-left px-3 py-2 text-sm text-destructive hover:bg-destructive/10 flex items-center gap-2">
                        ...
                        {aud.audiencia_status === 'agendada' && diff >= 0 && (
                          <button onClick={() => handleRegistrarResultado(aud)} className="border border-border text-secondary-foreground hover:bg-muted rounded-md px-3 py-1 text-xs flex items-center gap-1">
      ...
      <div className="relative bg-card rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col mx-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              <label className="block text-xs font-medium text-secondary-foreground mb-1">Tipo de Audiência*</label>
              <div className="flex gap-2 flex-wrap">
                {audTipoOptions.map((t) => (
                  <button key={t} type="button" onClick={() => setAudTipo(t)}
                    className={`rounded-md px-3 py-2 text-sm border transition-colors ${audienciaTipo === t ? 'bg-primary border-primary text-white' : 'bg-card border-border text-secondary-foreground'}`}>
            ...
            <div className="col-span-2 relative">
              <label className="block text-xs font-medium text-secondary-foreground mb-1">Processo</label>
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                ...
                <div className="absolute z-10 w-full bg-card border border-border rounded-lg shadow-md max-h-48 overflow-y-auto mt-1">
                  {filteredP.map((p) => (
                    <div key={p.id} onClick={() => selectProcesso(p)} className="hover:bg-muted px-3 py-2 cursor-pointer">
                      ...
                      <span className="text-sm text-foreground/80 ml-2">{p.acao}</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div>
              <label className="block text-xs font-medium text-secondary-foreground mb-1">Data*</label>
            ...
            <div>
              <label className="block text-xs font-medium text-secondary-foreground mb-1">Hora Início*</label>
              <input type="time" value={horaInicio} onChange={(e) => setHoraInicio(e.target.value)} className={fc('horaInicio')} />
            </div>

            <div>
              <label className="block text-xs font-medium text-secondary-foreground mb-1">Hora Fim</label>
            ...
            <div>
              <label className="block text-xs font-medium text-secondary-foreground mb-1">Vara/Tribunal</label>
            ...
            <div className="col-span-2">
              <label className="block text-xs font-medium text-secondary-foreground mb-1">Observações</label>
        ...
        <div className="border-t border-border px-6 py-4 flex items-center justify-end gap-3 flex-shrink-0">
          <button onClick={onClose} className="px-4 py-2 text-sm text-secondary-foreground hover:bg-muted rounded-md">Cancelar</button>
          <button onClick={handleSave} className="bg-primary text-white rounded-md px-6 py-2 text-sm font-medium hover:bg-primary/90">Salvar Audiência</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="relative bg-card rounded-xl shadow-xl max-w-md w-full mx-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-5">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-secondary-foreground mb-1">Resultado da Audiência*</label>
            ...
            <div>
              <label className="block text-xs font-medium text-secondary-foreground mb-1">Data de Realização*</label>
            ...
            <div>
              <label className="block text-xs font-medium text-secondary-foreground mb-1">Detalhes / Observações*</label>
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-border/50">
          <button onClick={onClose} className="flex-1 border border-border rounded-md py-2 text-sm text-secondary-foreground hover:bg-muted">Cancelar</button>
          <button onClick={handleSave} className="flex-1 bg-badge-ativo text-badge-ativo-foreground rounded-md py-2 text-sm font-medium hover:bg-badge-ativo/90">Registrar</button>
        </div>
      </div>
    </div>
  );
