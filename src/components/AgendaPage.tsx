                      ...
                      <div className="px-4 py-3">
                        ...
                        {evt.local && (
                          <div className="mt-1 flex items-center">
                            <MapPin className="w-3 h-3 text-muted-foreground/80 inline mr-1" />
              ...
              {upcomingEvents.length === 0 ? (
                ...
                upcomingEvents.map((evt) => (
                  <div key={evt.id} className="flex items-center gap-2 py-1.5">
                    ...
                    <span className="text-xs text-secondary-foreground truncate">{evt.title}</span>
                  </div>
                ))
              )}
            </div>
        ...
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center" onClick={() => setShowDeleteConfirm(null)}>
          <div className="bg-card rounded-xl shadow-xl p-6 max-w-sm mx-4 text-center" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowDeleteConfirm(null)} className="flex-1 border border-border rounded-md py-2 text-sm text-secondary-foreground">Cancelar</button>
      ...
      <div className="relative bg-card rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col mx-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5">
          <div className="grid grid-cols-2 gap-4">
            ...
            {/* Tipo */}
            <div className="col-span-2">
              <label className="block text-xs font-medium text-secondary-foreground mb-1">Tipo*</label>
                ...
                {(['audiencia', 'reuniao', 'prazo', 'pericia', 'outro'] as EventoTipo[]).map((t) => (
                  <button
                    ...
                    className={`rounded-md px-3 py-2 text-sm border transition-colors ${
                      tipo === t ? tipoSelectedColors[t] : 'bg-card border-border text-secondary-foreground'
                    }`}
                  >
                    {tipoLabels[t]}
                  </button>
            ...
            {/* Local */}
            <div>
              <label className="block text-xs font-medium text-secondary-foreground mb-1">Local</label>
        ...
        <div className="border-t border-border px-6 py-4 flex items-center justify-end gap-3 flex-shrink-0">
          ...
          <button onClick={handleSave} className="bg-primary text-white rounded-md px-6 py-2 text-sm font-medium hover:bg-primary/90">
      ...
      <div className="relative bg-card rounded-xl shadow-xl w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className={`px-6 py-4 border-b border-border border-l-4 ${tipoBorderColors[evento.tipo]}`}>
          <div className="flex items-start justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={onEdit} className="text-muted-foreground hover:text-secondary-foreground"><Edit className="w-4 h-4" /></button>
              <button onClick={onClose} className="text-muted-foreground hover:text-secondary-foreground"><X className="w-4 h-4" /></button>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            {evento.cliente_nome && (
              <div>
                <div className="flex items-center text-sm text-foreground/80">
                  <User className="w-4 h-4 text-muted-foreground/80 mr-1.5" />
                  {evento.cliente_nome}
                </div>
              </div>
            )}
            {lawyer && (
              <div>
                <div className="flex items-center text-sm text-foreground/80 gap-1.5">
                  <div className={`w-5 h-5 rounded-full ${lawyer.avatar_color} flex items-center justify-center text-white text-[9px] font-bold`}>
                    {lawyer.avatar_initials}
                  </div>
                  {lawyer.name}
          ...
          {evento.notes && (
            <>
              ...
              <div>
                ...
                <p className="text-sm text-secondary-foreground italic leading-relaxed mt-1">{evento.notes}</p>
              </div>
            </>
          )}
        </div>
        ...
        <div className="px-6 py-4 border-t border-border/50 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={onClose} className="border border-border text-secondary-foreground rounded-md px-4 py-2 text-sm">Fechar</button>
            <button onClick={onEdit} className="bg-primary text-white rounded-md px-4 py-2 text-sm font-medium hover:bg-primary/90">Editar Evento</button>
          </div>
        </div>
      </div>
    </div>
