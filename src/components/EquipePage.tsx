...
export default function EquipePage() {
  ...
  const selectClass = 'bg-card border border-border rounded-md px-3 py-2 text-sm text-secondary-foreground focus:outline-none focus:ring-2 focus:ring-ring';
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {admin && (
          <button onClick={() => { resetInvite(); setInviteOpen(true); }} className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-white text-sm font-medium rounded-md px-4 py-2 transition-colors">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-card border border-border shadow-sm rounded-lg px-4 py-3 flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-muted/80 text-secondary-foreground"><Users className="w-4 h-4" /></div>
        ...
        <div className="bg-card border border-border shadow-sm rounded-lg px-4 py-3 flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-primary/10 text-primary"><Briefcase className="w-4 h-4" /></div>
        ...
        <div className="bg-card border border-border shadow-sm rounded-lg px-4 py-3 flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-accent/10 text-accent"><Scale className="w-4 h-4" /></div>
        ...
        <div className="bg-card border border-border shadow-sm rounded-lg px-4 py-3 flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-badge-ativo/20 text-badge-ativo-foreground"><LayoutGrid className="w-4 h-4" /></div>
      ...
      {filteredUsers.length === 0 && filteredPending.length === 0 ? (
        ...
        <div className="grid grid-cols-3 gap-4">
          {filteredUsers.map(user => {
            ...
            return (
              <div key={user.id} className={`bg-card border rounded-lg p-5 hover:shadow-sm transition-shadow duration-150 flex flex-col gap-4 ${isMe ? 'border-primary/30 bg-primary/10/30' : 'border-border'}`}>
                {/* Top */}
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5">
                        ...
                        {isMe && <span className="bg-primary/20 text-primary text-xs px-1.5 py-0.5 rounded-full">Você</span>}
                      </div>
                  ...
                  {admin && (
                    <div className="relative">
                      <button onClick={() => setOpenDropdown(openDropdown === user.id ? null : user.id)} className="text-muted-foreground hover:text-secondary-foreground p-1"><MoreHorizontal className="w-4 h-4" /></button>
                      {openDropdown === user.id && (
                        <>
                          <div className="fixed inset-0 z-10" onClick={() => setOpenDropdown(null)} />
                          <div className="absolute right-0 top-8 bg-card border border-border rounded-lg shadow-lg py-1 w-48 z-20">
                            <button className="w-full px-3 py-2 text-sm text-foreground/80 hover:bg-muted flex items-center gap-2 text-left" onClick={() => { setOpenDropdown(null); showToast('Funcionalidade em desenvolvimento', 'info'); }}><Edit className="w-3.5 h-3.5" /> Editar Membro</button>
                            <button className="w-full px-3 py-2 text-sm text-foreground/80 hover:bg-muted flex items-center gap-2 text-left" onClick={() => { setOpenDropdown(null); }}><Briefcase className="w-3.5 h-3.5" /> Ver Processos</button>
                            {!isMe && (
                              <>
                                <div className="border-t border-border/50 my-1" />
                                <button className="w-full px-3 py-2 text-sm text-destructive hover:bg-destructive/10 flex items-center gap-2 text-left" onClick={() => { setOpenDropdown(null); showToast('Funcionalidade em desenvolvimento', 'info'); }}><UserX className="w-3.5 h-3.5" /> Remover do Escritório</button>
                              </>
                            )}
                          </div>
                        </>
                ...
                <div className="flex gap-2 mt-1">
                  <button onClick={() => setProfileUser(user)} className="flex-1 border border-border rounded-md py-2 text-sm text-secondary-foreground hover:bg-muted text-center cursor-pointer">Ver Perfil</button>
                  <button className="flex-1 border border-border rounded-md py-2 text-sm text-secondary-foreground hover:bg-muted text-center cursor-pointer flex items-center justify-center gap-1">
          ...
          {filteredPending.map(user => (
            <div key={user.id} className="bg-muted border border-dashed border-border rounded-lg p-5 opacity-80 flex flex-col gap-4">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-secondary text-muted-foreground flex items-center justify-center text-xl font-semibold">?</div>
                  <div>
                    <span className="text-lg font-semibold text-foreground">{user.name}</span>
                    <div className="mt-1"><span className="bg-badge-pendente text-badge-pendente-foreground text-xs font-medium px-2 py-0.5 rounded-full inline-flex items-center gap-1"><Clock className="w-3 h-3" /> Convite Pendente</span></div>
                  </div>
                </div>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {user.practice_areas.map(a => (
                  <span key={a} className={`${areaColors[a] || 'bg-muted/80 text-secondary-foreground'} text-xs font-medium px-2 py-0.5 rounded-full`}>{areaLabels[a] || a}</span>
                ))}
              </div>
              <div className="space-y-1.5">
                <div className="flex items-center gap-2"><Mail className="w-3.5 h-3.5 text-muted-foreground/80" /><span className="text-sm text-secondary-foreground">{user.email}</span></div>
              ...
              <div className="flex gap-2 mt-1">
                <button onClick={() => showToast('Convite reenviado', 'success')} className="flex-1 border border-border rounded-md py-2 text-sm text-secondary-foreground hover:bg-muted text-center">Reenviar</button>
                <button onClick={() => removePending(user.id)} className="flex-1 border border-destructive/20 rounded-md py-2 text-sm text-destructive hover:bg-destructive/10 text-center">Cancelar convite</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={() => setProfileUser(null)}>
          ...
          <div className="bg-card rounded-xl shadow-xl max-w-md mx-4 w-full relative z-10 overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {(() => {
                      ...
                      return (<>
                        <div className="bg-badge-pendente/20 rounded-lg p-3"><div className="text-xs text-badge-pendente-foreground">A Receber</div><div className="text-lg font-bold text-badge-pendente-foreground">{formatBRL(f.aReceber)}</div></div>
                        <div className="bg-badge-ativo/20 rounded-lg p-3"><div className="text-xs text-badge-ativo-foreground">Recebido</div><div className="text-lg font-bold text-badge-ativo-foreground">{formatBRL(f.recebido)}</div></div>
                      </>);
                    })()}
                  </div>
                </>
            ...
            <div className="px-6 py-4 border-t border-border/50 flex gap-3">
              <button onClick={() => setProfileUser(null)} className="flex-1 border border-border rounded-md py-2 text-sm text-secondary-foreground">Fechar</button>
              {admin && profileUser.id !== currentUser!.id && (
                <button onClick={() => { setProfileUser(null); showToast('Funcionalidade em desenvolvimento', 'info'); }} className="flex-1 bg-primary text-white rounded-md py-2 text-sm font-medium hover:bg-primary/90">Editar Membro</button>
              )}
              {profileUser.id === currentUser!.id && (
                <button onClick={() => { setProfileUser(null); }} className="flex-1 bg-primary text-white rounded-md py-2 text-sm font-medium hover:bg-primary/90">Editar Meu Perfil</button>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={() => setInviteOpen(false)}>
          ...
          <div className="bg-card rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col mx-4 relative z-10" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
              ...
              <button onClick={() => setInviteOpen(false)} className="text-muted-foreground hover:text-secondary-foreground"><X className="w-5 h-5" /></button>
            </div>
            <div className="overflow-y-auto flex-1 px-6 py-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="text-sm font-medium text-foreground/80 block mb-1">Nome Completo*</label>
                ...
                <div className="col-span-2">
                  <label className="text-sm font-medium text-foreground/80 block mb-1">Email*</label>
                ...
                <div>
                  <label className="text-sm font-medium text-foreground/80 block mb-1">OAB</label>
                ...
                <div>
                  <label className="text-sm font-medium text-foreground/80 block mb-1">Cargo/Role*</label>
                ...
                <div className="col-span-2">
                  <label className="text-sm font-medium text-foreground/80 block mb-2">Áreas de Atuação*</label>
                  <div className="grid grid-cols-2 gap-2">
                    {['trabalhista', 'civil', 'criminal', 'previdenciario'].map(a => (
                      <button key={a} type="button" onClick={() => toggleArea(a)} className={`flex items-center gap-2.5 border rounded-lg px-3 py-2.5 cursor-pointer hover:bg-muted transition-colors ${invAreas.includes(a) ? 'border-primary/40 bg-primary/10' : 'border-border'}`}>
                        <div className={`w-4 h-4 rounded border-2 flex items-center justify-center ${invAreas.includes(a) ? 'bg-primary border-primary' : 'border-border bg-card'}`}>
                          {invAreas.includes(a) && <Check className="w-3 h-3 text-white" />}
                        </div>
                        <span className="text-sm text-foreground/80">{areaLabels[a]}</span>
                ...
                <div className="col-span-2">
                  <label className="text-sm font-medium text-foreground/80 block mb-1">Telefone</label>
                ...
                {invEmail && invName && (
                  <div className="col-span-2 bg-primary/10 border border-primary/20 rounded-lg p-4">
                    <div className="text-xs font-semibold text-primary uppercase mb-2">Prévia do convite</div>
                    <div className="bg-card rounded-lg p-3 border border-primary/20">
                      <div className="flex items-center gap-1.5"><Mail className="w-4 h-4 text-primary/80" /><span className="text-sm text-foreground/80">Convite para: {invEmail}</span></div>
                      <div className="text-xs text-muted-foreground mt-0.5">{invName} foi convidado como {roleLabels[invRole] || invRole}</div>
                      {invAreas.length > 0 && (
                        <div className="flex gap-1 mt-1">
                          {invAreas.map(a => <span key={a} className={`${areaColors[a] || 'bg-muted/80 text-secondary-foreground'} text-xs font-medium px-2 py-0.5 rounded-full`}>{areaLabels[a] || a}</span>)}
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="border-t border-border px-6 py-4 flex items-center justify-between flex-shrink-0">
              ...
              <div className="flex gap-3">
                <button onClick={() => setInviteOpen(false)} className="text-secondary-foreground text-sm px-3 py-2">Cancelar</button>
                <button onClick={handleInvite} className="bg-primary text-white rounded-md px-6 py-2 text-sm font-medium hover:bg-primary/90 flex items-center gap-1.5"><Send className="w-4 h-4" /> Enviar Convite</button>
              </div>
            </div>
          </div>
        </div>
